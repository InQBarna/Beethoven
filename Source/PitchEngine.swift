...
public final class PitchEngine {
    ...
    public var levelThreshold: Float? {
        get {
            return signalTracker.levelThreshold
        }
        set {
            signalTracker.levelThreshold = newValue
        }
    }
...
extension PitchEngine: SignalTrackerDelegate {
    public func signalTracker(_: SignalTracker,
        ...
        queue.async { [weak self] in
            ...
            do {
                ...
                DispatchQueue.main.async { [weak self] in
                    guard let `self` = self else { return }
                    self.delegate?.pitchEngine(self, didReceivePitch: pitch, rmsLevel: rmsLevel, mfccs: mfccs, at: time)
                }
            } catch {
                DispatchQueue.main.async { [weak self] in
                    guard let `self` = self else { return }
    ...
    public func signalTrackerWentBelowLevelThreshold(_: SignalTracker, rmsLevel: Float, mfccs: [Float], atTime time: AVAudioTime) {
        DispatchQueue.main.async {
            self.delegate?.pitchEngineWentBelowLevelThreshold(self, rmsLevel: rmsLevel, mfccs: mfccs, at: time)
        }
    }
}
