...
final class YINUtil {
    ...
    class func difference_broken_do_not_use(buffer: [Float]) -> [Float] {
        ...
        temp.withMemoryRebound(to: DSPComplex.self, capacity: buffer.count) { typeConvertedTransferBuffer in
        ...
        ktemp.withMemoryRebound(to: DSPComplex.self, capacity: kernel.count) { typeConvertedTransferBuffer in
